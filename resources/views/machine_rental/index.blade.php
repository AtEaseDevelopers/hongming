...
@section('content')
    <ol class="breadcrumb">
        <li class="breadcrumb-item">Machine Rental</li>
    </ol>
                 ...
                 <div class="col-lg-12">
                     <div class="card">
                         <div class="card-header">
                             ...
                             Machine Rental
                             <a class="pull-right" href="{{ route('machineRentals.create') }}"><i class="fa fa-plus-square fa-lg"></i></a>
                         </div>
                         <div class="card-body">
                             @include('machine_rental.table')
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/machineRentals/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/machineRentals/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/machineRentals/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status
